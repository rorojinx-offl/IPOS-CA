...
public class CustomerMonthlySpend {
    static void main() {
        ...
        try {
            ddl.createTable(conn,"customer_monthly_spend",initiateTables());
            ddl.createIndex(conn,"customer_monthly_spend","CUST_ID");
    ...
    public static void create(Connection conn, DDLEngine ddl) throws SQLException {
        ddl.createTable(conn, "customer_monthly_spend", CustomerMonthlySpend.initiateTables());
        ddl.createIndex(conn,"customer_monthly_spend","CUST_ID");
    }
        ...
        return new ArrayList<>() {
            {
                ...
                add(new TableSchema.Column("SPEND", "REAL", false, true,new ArrayList<>() {{
                    add(new ColumnConstraint.Default("0"));
                }}));
                add(new TableSchema.MultiPrimaryKey("CUST_ID, MONTH_YEAR"));
